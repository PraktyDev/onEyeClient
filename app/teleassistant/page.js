...
const page = () => {
  ...
  const onSubmit = async (values) => {
    try {
      ...
      router.push('/teleassistant/dashboard')
      toast("Login Success")
    } catch (error) {
      toast("Login failed")
    }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 text-[#0A416F] p-4">
      ...
      <p className="text-[#0A416F] font-medium my-3">TeleAssistant's Login</p>
          ...
          <div className="flex flex-col tablet:flex-row gap-6">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="TeleAssistant Praise" {...field} />
                        </FormControl>
                        <FormDescription>
            ...
            <div className="flex-1 hidden tablet:flex items-center justify-center -my-6 -mr-6">
              <Image
                ...
                height={1000}
                priority={true}
                src="/images/teleassist.jpg"
                alt="doctor login image"
                className=" max-w-full h-[450px] rounded-tl-none rounded-bl-none rounded-lg shadow-lg object-cover"
              />
            </div>
          </div>
        </CardContent>
